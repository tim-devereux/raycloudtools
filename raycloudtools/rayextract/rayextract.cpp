...
void usage(int exit_code = 1)
{
  ...
  if (extract_type == "trees" || none)
  {
    ...
    std::cout << "                            --segment_alpha_weighting - use point cloud alpha as weight for cylinder radius fitting" << std::endl;
...
int rayExtract(int argc, char *argv[])
{
  ...
  ray::OptionalFlagArgument exclude_rays("exclude_rays", 'e'), segment_branches("branch_segmentation", 'b'), stalks("stalks", 's'), use_rays("use_rays", 'u'),
    alpha_weighted("alpha_weighting", 'p'), segment_alpha_weighted("segment_alpha_weighting", '\0');
  ...
  bool extract_trees = ray::parseCommandLine(
    ...
    { &max_diameter_option, &distance_limit_option, &height_min_option, &crop_length_option, &girth_height_ratio_option,
      ...
      &segment_branches, &grid_width_option, &global_taper_option, &global_taper_factor_option, &use_rays, &alpha_weighted, &segment_alpha_weighted, &verbose, &view_flag });
  ...
  else if (extract_trees)
  {
    ...
    params.segment_alpha_weighting = segment_alpha_weighted.isSet();
